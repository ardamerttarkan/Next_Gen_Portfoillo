...
export const BlogDetail: React.FC<BlogDetailProps> = ({ post, onBack, theme }) => {
  ...
  // Theme Styles
  const bgClass = isProf ? 'bg-slate-50 dark:bg-slate-950' : 'bg-gray-50 dark:bg-[#0a0a0a]';
  const textClass = isProf ? 'text-slate-900 dark:text-white' : 'text-gray-900 dark:text-white';
  const accentClass = isProf ? 'text-prof-blue' : 'text-yellow-600 dark:text-yellow-500';
  ...
  return (
    <div className={`min-h-screen ${bgClass} font-sans transition-colors duration-300`}>
      ...
      {/* Navigation Bar */}
      <div className={`sticky top-0 z-50 backdrop-blur-md bg-opacity-80 border-b ${isProf ? 'bg-white/80 dark:bg-slate-900/80 border-slate-200 dark:border-slate-800' : 'bg-white/80 dark:bg-black/50 border-gray-200 dark:border-white/10'}`}>
      ...
      <div className="w-full h-[40vh] relative">
        ...
        <div className="absolute bottom-0 left-0 right-0 max-w-4xl mx-auto px-4 pb-12">
            <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider mb-4 ${isProf ? 'bg-blue-600 text-white' : 'bg-yellow-500 text-black'}`}>
              {post.category}
            </span>
      ...
      <article className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className={`prose prose-lg dark:prose-invert max-w-none ${textClass}`}>
          <p className="font-medium text-xl leading-relaxed mb-8 opacity-90">{post.excerpt}</p>
